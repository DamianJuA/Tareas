...
public class ModeloTablaInstrumento implements TableModel {
    ...
    public void eliminarDatos(int e){
        if (e>= 0 && e < datos.size()) {
            InstrumentoMusical instrumento = datos.get(e);
            try {
                if (ldao.delet(Integer.toString(instrumento.getId()))) {
                    datos.remove(e);
                    eliminarDatos(e);
                } else {
    ...
    public void ActualizarDatos(int o, InstrumentoMusical inst) {
        if (0 >= 0 && o < datos.size()) {
            InstrumentoMusical nuevo = datos.get(o);
            nuevo.setNombre(inst.getNombre());
            nuevo.setMaterial(inst.getMaterial());
            nuevo.setTipo(inst.getTipo());
            nuevo.setDedosopua(inst.getDedosopua());
            nuevo.setURL(inst.getURL());
            try {
                ldao.update(nuevo);
            } catch (SQLException sqle) {
                System.out.println(sqle.getMessage());
